...
describe('formatErrorForUser', () => {
  ...
  test('returns user-friendly message for OpenRouter authentication errors', () => {
    const message =
      '401 User not found. Troubleshooting URL: https://docs.langchain.com/oss/javascript/langchain/errors/MODEL_AUTHENTICATION/';

    expect(formatErrorForUser(new Error(message))).toBe(
  ...
  test('returns user-friendly message for missing OpenRouter API key errors', () => {
    expect(formatErrorForUser(new Error('OpenRouter API key is missing.'))).toBe(
      'OpenRouter authentication failed. Check that OPENROUTER_API_KEY is set to a valid key for your OpenRouter account, then restart BBallGenius Chat.',
    );
  });
