import { getColumns, getTableRefs, type TableRef } from '../db.js';
import {
...
function resolveBestQualifiedName(tablePattern: string, tables: TableRef[]): string {
...
export async function buildIntentSchemaPrompt(intentCategory: string): Promise<string | null> {
  ...
  const allTables = await getTableRefs();
  ...
  const tableSections = await Promise.all(
    tablePatterns.map(async (pattern) => {
      const qualifiedName = resolveBestQualifiedName(pattern, allTables);
