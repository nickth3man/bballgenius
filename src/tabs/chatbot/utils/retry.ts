...
function isOpenRouterAuthError(message: string): boolean {
  ...
  if (msg.includes('openrouter api key is missing')) return true;
  if (!msg.includes('401')) return false;
  return (
    msg.includes('user not found') ||
    msg.includes('model_authentication') ||
...
export function isRetryableError(err: unknown): boolean {
  return categorizeDbError(err) === 'transient';
}

export function formatErrorForUser(err: unknown): string {
  if (!(err instanceof Error)) return String(err);
  if (isOpenRouterAuthError(err.message)) return OPENROUTER_AUTH_ERROR;
  const msg = err.message.toLowerCase();
