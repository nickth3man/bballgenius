...
import { ChatbotTab } from './chatbot/index.js';
...
const CHATBOT_SHORTCUTS: KeyboardShortcut[] = [
  { keys: 'Type', action: 'Ask NBA questions' },
  { keys: 'Enter', action: 'Send message' },
  { keys: 'Tab', action: 'Cycle input / chat history' },
  { keys: '@ / Ctrl+P', action: 'Open model selector' },
  { keys: 'Esc', action: 'Cancel processing, or blur input' },
];
...
export const TAB_REGISTRY: TabDefinition[] = [
  ...
  },
  {
    ...
    name: 'Chat',
    shortcutIndex: 4,
    keyboardShortcuts: CHATBOT_SHORTCUTS,
    create: (renderer: CliRenderer) => new ChatbotTab(renderer),
  },
];
...
export function buildFooterShortcutsHint(registry: TabDefinition[]): string {
  ...
  return `F${first}-${last} Nav Tab ? Esc`;
}
