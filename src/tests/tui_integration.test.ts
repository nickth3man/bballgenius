...
describe('TUI Hub Shell & Navigation Integration Tests', () => {
  ...
  test('should boot application, render header tabs, and toggle visibility on navigation keys', async () => {
    ...
    // 2. Build the root container mirroring src/index.ts
    ...
    expect(tabs[0].container.visible).toBe(true);
    expect(tabs[1].container.visible).toBe(false);
    ...
    expect(tabs[0].container.visible).toBe(false);
    expect(tabs[1].container.visible).toBe(false);
    expect(tabs[2].container.visible).toBe(true);
  ...
  test('should handle focus cycling inside active tabs', async () => {
    ...
    // Programmatically cycle focus to Stats panel (Index 1)
  ...
  test('should handle interactive schema table expansion and column rendering on SqlSandboxTab', async () => {
    ...
    expect(sqlSandbox['schemaNodes'].length).toBeGreaterThan(0);
    expect(sqlSandbox['selectedSchemaIdx']).toBe(0);

    // Capture initial count of displayed schema elements
    const initialNodesCount = sqlSandbox['schemaNodes'].length;

    // Target the first element (which should be a "table" type node)
    const firstNode = sqlSandbox['schemaNodes'][0];
    ...
    expect(sqlSandbox['expandedTables'].has(firstNode.name)).toBe(true);

    // Verify that column nodes were successfully injected under the expanded table
    const expandedNodesCount = sqlSandbox['schemaNodes'].length;
...
describe.serial('TUI accessibility integration (intended app shell API)', () => {
  ...
  test('Shift+Tab via mockInput cycles focus backward on Time Machine', async () => {
    ...
    // Esc blurs search and focuses stats; Tab is blocked while search input is focused.
