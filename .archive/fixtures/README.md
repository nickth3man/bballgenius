...
`nba.ci.duckdb` is a **minimal subset** of the full NBA database used in GitHub Actions and local fixture-based test commands. It is committed to git (~3 MB) so PRs can run hub integration tests and chatbot SQL tests without the 1.5 GB `data/nba.duckdb`.
...
| Related teams, box scores, PBP, awards, season stats | All queries under `src/hub/tabs/*/queries.ts` |
...
`src/shared/dbPath.ts` → `resolveDbPath()`:
...
CI sets `NBA_DUCKDB_PATH=data/fixtures/nba.ci.duckdb` explicitly in [`.github/workflows/ci.yml`](../../.github/workflows/ci.yml) for hub and chatbot test jobs.
...
4. Commit `nba.ci.duckdb` and any updated snapshot under `src/hub/tests/snapshots/`.
